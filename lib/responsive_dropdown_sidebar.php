<?php
class floris_ResmenuSB{
	function __construct(){
		add_filter( 'wp_nav_menu_args' , array( $this , 'floris_MenuRes_AdFilter' ), 100 ); 
		add_filter( 'wp_nav_menu_args' , array( $this , 'floris_MenuRes_Filter' ), 110 );	
		add_action( 'wp_footer', array( $this, 'floris_MenuRes_AdScript' ), 110 );
		add_action( 'wp_footer', array( $this, 'floris_responsive_menu_footer' ) );	
	}
	function floris_MenuRes_AdScript(){
	...
	function floris_MenuRes_AdFilter( $args ){
		$args['container'] = false;
		$menu = get_registered_nav_menus() ;
		foreach( $menu as $floris_theme_locate => $menu_description ){
			if( $floris_theme_locate == $args['theme_location'] ){
				if( isset( $args['floris_resmenu'] ) && $args['floris_resmenu'] == true ) {
					return $args;
				}		
				$ResNavMenu = $this->ResNavMenu( $args );
				$args['container'] = '';
				$args['container_class'].= '';	
				$args['menu_class'].= ($args['menu_class'] == '' ? '' : ' ') . 'floris-menures';			
	...
	function ResNavMenu( $args ){
		$args['floris_resmenu'] = true;		
		$select = wp_nav_menu( $args );
		return $select;
	}
	function floris_MenuRes_Filter( $args ){
		/* Fix Menu on wp 4.7 */
		if( !isset( $args['floris_resmenu'] ) ){
			return $args;
		}
		$menu = get_registered_nav_menus() ;
		
			if( in_array( $args['theme_location'], array( 'primary_menu', 'vertical_menu', 'mobile_header_menu' ) ) ){	
				$args['container'] = 'div';
	...
	function floris_responsive_menu_footer(){
						...
						<ul class="nav nav-tabs">
							...
							<?php if( has_nav_menu('primary_menu') ) : ?>
								<li class="<?php echo esc_attr( $class_p ); ?>">
									<a href="#ResPrimary" data-toggle="tab" class="tab-primary"><?php echo esc_html__( 'Menu', 'floris' ); ?></a>
								</li>
							<?php endif; ?>
								
							<?php if( has_nav_menu('vertical_menu') ) : ?>
								<li class="<?php echo esc_attr( $class_v ); ?>">
									<a href="#ResVertical" data-toggle="tab" class="tab-vertical"><?php echo esc_html__( 'Categories', 'floris' ); ?></a>
								</li>
							<?php endif; ?>
							<?php } ?>
						</ul>
									...
									<?php if( has_nav_menu( $location ) ) : ?>
										<?php 
											wp_nav_menu( array(
												...
												'walker' => new Floris_ResMenu_Walker()
											) );
										?>
									<?php endif; ?>
								</div>
...
class Floris_ResMenu_Walker extends Walker_Nav_Menu {
