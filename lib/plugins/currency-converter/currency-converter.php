...
if ( ! class_exists( 'WC_Currency_Converter' ) ) {
	...
	class WC_Currency_Converter {
		...
		public function __construct() {
			$this->settings = array(
				array(
					'name' => esc_html__( 'Open Exchange Rate API', 'floris' ),
					'type' => 'title',
					'desc' => '',
					'id'   => 'woocommerce-currency-converter-widget'
				),
				array(
					'name' => esc_html__( 'App Key', 'floris' ),
					'desc' => sprintf( __( '(optional) If you have an <a href="%s">Open Exchange Rate API app ID</a>, enter it here.', 'floris' ), 'https://openexchangerates.org/signup' ),
		...
		public function admin_notice_wrong_key() {
			?>
			<div class="error">
				<p><?php esc_html_e( 'WooCommerce Currency Converter: Incorrect key entered!', 'floris' ); ?></p>
			</div>
		<?php
		}
		...
		public function load_plugin_textdomain() {
			load_plugin_textdomain( 'floris', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
		}
			...
			if ( $currencies ) {
				...
				if ( ! empty( $instance['currency_display'] ) && 'select' === $instance['currency_display'] ) {
					...
					if ( ! empty( $instance['show_reset'] ) ) {
						$html .= ' <a href="#" class="wc-currency-converter-reset reset">' . esc_html__( 'Reset', 'floris' ) . '</a>';
					}
					...
					if ( ! empty( $instance['show_reset'] ) ) {
						$html .= '<li><a href="#" class="wc-currency-converter-reset reset">' . esc_html__( 'Reset', 'floris' ) . '</a></li>';
					}
		...
		public function enqueue_assets() {
			...
			$wc_currency_converter_params = array(
				...
				'i18n_oprice'            => esc_html__( 'Original price:', 'floris' ),
