...
*/
add_action( 'wcvendors_before_dashboard', 'floris_wrapper_before_vendor_dashboard' );
add_action( 'wcvendors_after_dashboard', 'floris_wrapper_after_vendor_dashboard' );
function floris_wrapper_before_vendor_dashboard(){
	echo '<div class="vendor-dashboard-wrapper">';
}

function floris_wrapper_after_vendor_dashboard(){
	echo '</div>';
}

add_action( 'wp', 'floris_wcvendor_hook' );
function floris_wcvendor_hook(){
	...
	if( 'pro' !== $pro_store_header ) {
		...
		if( WCV_Vendors::is_vendor_page() ) {
			add_action( 'woocommerce_before_main_content', 'floris_vendor_breadcrumb', 9 );
...
function floris_vendor_breadcrumb(){
?>
...
** Add wrapper sold by
*/
add_filter( 'wcvendors_sold_by_in_loop', 'floris_custom_soldby' );
function floris_custom_soldby( $sold_by_label ){
