 ...
 * Page titles
 */
function floris_title() {
	if (is_home()) {
		if (get_option('page_for_posts', true)) {
			echo get_the_title(get_option('page_for_posts', true));
		} else {
			esc_html_e('Latest Posts', 'floris');
		}
	} elseif (is_archive()) {
		...
		if ($term) {
			...
			printf(__('Author Archives: %s', 'floris'), get_the_author());
		} else {
			single_cat_title();
		}
	} elseif (is_search()) {
		printf(__('Search Results for <small>%s</small>', 'floris'), get_search_query());
	} elseif (is_404()) {
		esc_html_e('Not Found', 'floris');
	} else {
		the_title();
	}
}
 ...
 */
function floris_leadingslashit($string) {
	return '/' . floris_unleadingslashit($string);
}

function floris_unleadingslashit($string) {
	return ltrim($string, '/');
}

function floris_element_empty($element) {
...
?>
	<div class="social-share">
		<div class="title-share"><?php esc_html_e( 'Share','floris' ) ?></div>
...
function floris_get_avatar($avatar) {
	$avatar = str_replace("class='avatar", "class='avatar pull-left media-object", $avatar);
	return $avatar;
}
add_filter('get_avatar', 'floris_get_avatar');


/*
** Check col for sidebar and content product
*/
function floris_content_product(){ 
...
*/
function floris_content_product_detail(){
	$left_span_class 			= sw_options('sidebar_left_expand');
	$left_span_md_class 	= sw_options('sidebar_left_expand_md');
	$left_span_sm_class 	= sw_options('sidebar_left_expand_sm');
	$right_span_class 		= sw_options('sidebar_right_expand');
...
** Check sidebar blog
*/
function floris_sidebar_template(){
	$floris_sidebar_teplate = sw_options('sidebar_blog');
	if( !is_archive() ){
		$floris_sidebar_teplate = ( get_term_meta( get_queried_object()->term_id, 'term_sidebar', true ) != '' ) ? get_term_meta( get_queried_object()->term_id, 'term_sidebar', true ) : sw_options('sidebar_blog');
	}	
	if( is_single() ) {
		$floris_sidebar_teplate = ( get_post_meta( get_the_ID(), 'page_sidebar_layout', true ) != '' ) ? get_post_meta( get_the_ID(), 'page_sidebar_layout', true ) : sw_options('sidebar_blog');
	}
	return $floris_sidebar_teplate;
}

/*
** Check col for sidebar and content page
*/
function floris_content_page(){
	$left_span_class 			= sw_options('sidebar_left_expand');
	$left_span_md_class 	= sw_options('sidebar_left_expand_md');
	$left_span_sm_class 	= sw_options('sidebar_left_expand_sm');
	$right_span_class 		= sw_options('sidebar_right_expand');
...
** Typography
*/
function floris_typography_css(){
	$styles = '';
...
function floris_typography_css_cache(){ 
		
	/* Custom Css */
	if ( sw_options('advanced_css') != '' ){
		echo'<style>'. sw_options( 'advanced_css' ) .'</style>';
	}
	$data = floris_typography_css();
	echo ( $data );
}
add_action( 'wp_head', 'floris_typography_css_cache', 12, 0 );

function floris_typography_webfonts(){
...
function floris_googlefonts_script() {
    wp_enqueue_style( 'floris-googlefonts', floris_typography_webfonts(), array(), '1.0.0' );
}
add_action( 'wp_enqueue_scripts', 'floris_googlefonts_script' );


/* 
** Get video or iframe from content 
*/
function floris_get_entry_content_asset( $post_id ){
	global $post;
...
** Tag cloud size
*/
add_filter( 'widget_tag_cloud_args', 'floris_tag_clound' );
function floris_tag_clound($args){
	$args['largest'] = 8;
	return $args;
}
...
class floris_Breadcrumbs {
	...
	function breadcrumb( $before = '', $after = '', $display = true ) {
		$options = array('breadcrumbs-home' => esc_html__( 'Home', 'floris' ), 'breadcrumbs-blog-remove' => false, 'post_types-post-maintax' => '0');
		...
		$links = array(
			array(
				'url'  => get_home_url(),
				'text' => ( isset( $options['breadcrumbs-home'] ) && $options['breadcrumbs-home'] != '' ) ? $options['breadcrumbs-home'] : esc_html__( 'Home', 'floris' )
			)
		);
		...
		if ( ( $on_front == "page" && is_front_page() ) || ( $on_front == "posts" && is_home() ) ) {
			...
			if ( is_post_type_archive() ) {
				...
				$bc = esc_html__( 'Archives for', 'floris' );
				...
				$bc = esc_html__( 'You searched for', 'floris' );
				$links[] = array( 'text' => $bc . ' "' . esc_html( get_search_query() ) . '"' );
			} elseif ( is_404() ) {
				$crumb404 = esc_html__( 'Error 404: Page not found', 'floris' );
...
global $floris_breadcrumb;
$floris_breadcrumb = new floris_Breadcrumbs();
...
if ( !function_exists( 'floris_breadcrumb' ) ) {
	 ...
	 * @return string
	 */
	function floris_breadcrumb( $before = '', $after = '', $display = true ) {
		global $floris_breadcrumb;
		...
		return $floris_breadcrumb->breadcrumb( $before, $after, $display );
	}
}
...
** Footer Adnvanced
*/
add_action( 'wp_footer', 'floris_footer_advanced' );
function floris_footer_advanced(){
	/* 
	** Back To Top 
	*/
	if( sw_options( 'back_active' ) ) :
		echo '<a id="floris-totop" href="#" ></a>';
	endif;
	...
	*/
	if( sw_options( 'popup_active' ) ) :
		$floris_content = sw_options( 'popup_content' );
		$floris_shortcode = sw_options( 'popup_form' );
		...
		<div id="subscribe_popup" class="subscribe-popup">
			<div class="subscribe-popup-container clearfix">
				<div class="image-newsletter pull-left">
					<img src="<?php echo esc_url( sw_options( 'popup_background' ) )?>" />
				</div>
				<div class="subscribe-content pull-right">
					<?php if( $floris_content != '' ) : ?>
					<div class="popup-content">
						<?php echo ( $floris_content ); ?>
					</div>
					<?php endif; ?>
					
					<?php if( $floris_shortcode != '' ) : ?>
					<div class="subscribe-form">
						<?php	echo do_shortcode( $floris_shortcode ); ?>
					</div>
					<?php endif; ?>
					...
					<div class="subscribe-checkbox">
						<label for="popup_check">
							...
							<?php echo '<span>' . esc_html__( "Don't show this popup again!", "floris" ) . '</span>'; ?>
						</label>
					</div>
...
?>
	<div class="modal fade" id="login_form" tabindex="-1" role="dialog" aria-hidden="true">
		<div class="modal-dialog block-popup-login">
			<?php ob_start(); ?>
			<a href="javascript:void(0)" title="<?php esc_attr_e( 'Close', 'floris' ) ?>" class="close close-login" data-dismiss="modal"><?php esc_html_e( 'Close', 'floris' ) ?></a>
			<div class="tt_popup_login"><strong><?php esc_html_e('Sign in Or Register', 'floris'); ?></strong></div>
			<?php get_template_part('woocommerce/myaccount/login-form'); ?>
			<?php 
				if( class_exists( 'APSL_Class' ) ) :
				echo '<div class="login-line"><span>'. esc_html__( 'Or', 'floris' ) .'</span></div>';
				...
				$html = ob_get_clean();
				echo apply_filters( 'floris_custom_login_filter', $html );
			?>
		</div>
	</div>
<?php 	
	...
	<div class="modal fade" id="search_form" tabindex="-1" role="dialog" aria-hidden="true">
		<div class="modal-dialog block-popup-search-form">
			<form role="search" method="get" class="form-search searchform" action="<?php echo esc_url( home_url( '/' ) ); ?>">
				<input type="text" value="<?php if (is_search()) { echo get_search_query(); } ?>" name="s" class="search-query" placeholder="<?php esc_attr_e( 'Enter your keyword...', 'floris' ) ?>">
				<button type="submit" class=" fa fa-search button-search-pro form-button"></button>
				<a href="javascript:void(0)" title="<?php esc_attr_e( 'Close', 'floris' ) ?>" class="close close-search" data-dismiss="modal"><?php esc_html_e( 'X', 'floris' ) ?></a>
			</form>
		</div>
	</div>
<?php 
...
**/
function floris_advanced(){	
	$floris_popup	 		= sw_options( 'popup_active' );
	$sticky_mobile	 		= sw_options( 'sticky_mobile' );
	$output  = '';
	$output .= '(function($) {';
	if( !floris_mobile_check() ) : 
		$sticky_menu 		= sw_options( 'sticky_menu' );
		$floris_header_style 	= ( get_post_meta( get_the_ID(), 'page_header_style', true ) != '' ) ? get_post_meta( get_the_ID(), 'page_header_style', true ) : sw_options('header_style');
		$output_css = '';
		...
		if( $layout == 'boxed' ){
			...
			$output_css .= '}';
			wp_enqueue_style(	'floris_custom_css',	get_template_directory_uri() . '/css/custom_css.css' );
			wp_add_inline_style( 'floris_custom_css', $output_css );
		}
		...
		*/
		if( $sticky_menu ) :		
				if( $floris_header_style == 'style2' || $floris_header_style == ''){			
				...
				elseif( $floris_header_style == 'style1' ){
		...
		wp_add_inline_script( 'floris_custom_js', $output );
	
}
add_action( 'wp_enqueue_scripts', 'floris_advanced', 101 );


/**
* Set and Get view count
**/
function floris_getPostViews($postID){    
...
function floris_setPostViews($postID) {
	$count_key = 'post_views_count';
	$count = get_post_meta($postID, $count_key, true);
	if($count==''){
			$count = 0;
...
*/
add_action( 'wp_head', 'floris_create_postview' );
function floris_create_postview(){
	if( is_single() || is_singular( 'product' ) ) :
		floris_setPostViews( get_the_ID() );
	endif;
}
...
function floris_logo(){
	...
	$logo_select = ( floris_mobile_check() && $mobile_logo != ''  ) ? $mobile_logo : sw_options( 'sitelogo' );
...
add_action( 'save_post', 'floris_product_save_meta', 10, 1 );
function post_init(){
	add_meta_box("floris_product_meta", esc_html__( 'Product Meta:', 'floris' ), "floris_product_meta", "product", "side", "low");
	add_meta_box("floris_product_video_meta", esc_html__( 'Featured Video Product', 'floris' ), "floris_product_video_meta", "product", "side", "low");
}	
function floris_product_meta(){
	global $post;
	$recommend_product = get_post_meta( $post->ID, 'recommend_product', true );
	$newproduct 	   = get_post_meta( $post->ID, 'newproduct', true );
?>
	<p><label><b><?php esc_html_e( 'Recommend Product:', 'floris' ) ?></b></label> &nbsp;&nbsp;
	<input type="checkbox" name="recommend_product" value="1" <?php echo checked( $recommend_product, 1 ) ?> /></p>
	
	<p><label><b><?php esc_html_e( 'New Product', 'floris' ) ?></b></label> &nbsp;&nbsp;
		<input type="number" name="newproduct" value="<?php echo esc_attr( $newproduct ) ?>"/>
		<span class="p-description"><?php echo esc_html__( 'Set day for the new product label from the date publish product.', 'floris' ); ?></span>
	</p>
<?php }

function floris_product_video_meta(){
	global $post;
...
?>
	<div class="featured-image">
		...
		<?php endif; ?>
		<p><input type="text" name="featured_video_product" placeholder="<?php echo esc_attr__( 'Youtube Video ID', 'floris' ) ?>" value="<?php echo esc_attr( $featured_video_product ); ?>"/></p>
	</div>
<?php 
}

function floris_product_save_meta( $post_id ){
...
function floris_trim_words( $title ){
...
** Advanced Favico
*/
add_filter( 'get_site_icon_url', 'floris_site_favicon', 10, 1 );
function floris_site_favicon( $url ){
...
if( !function_exists( 'sw_social_link' ) ) {
	function sw_social_link(){
		...
		$html .= '<div class="floris-socials"><ul>';
			if( $fb_link != '' ):
				$html .= '<li><a href="'. esc_url( $fb_link ) .'" title="'. esc_attr__( 'Facebook', 'floris' ) .'"><i class="fa fa-facebook"></i></a></li>';
			endif;
			
			if( $tw_link != '' ):
				$html .= '<li><a href="'. esc_url( $tw_link ) .'" title="'. esc_attr__( 'Twitter', 'floris' ) .'"><i class="fa fa-twitter"></i></a></li>';
			endif;
			
			if( $tb_link != '' ):
				$html .= '<li><a href="'. esc_url( $tb_link ) .'" title="'. esc_attr__( 'Tumblr', 'floris' ) .'"><i class="fa fa-tumblr"></i></a></li>';
			endif;
			
			if( $li_link != '' ):
				$html .= '<li><a href="'. esc_url( $li_link ) .'" title="'. esc_attr__( 'Linkedin', 'floris' ) .'"><i class="fa fa-linkedin"></i></a></li>';
			endif;
			
			if( $it_link != '' ):
				$html .= '<li><a href="'. esc_url( $it_link ) .'" title="'. esc_attr__( 'Instagram', 'floris' ) .'"><i class="fa fa-instagram"></i></a></li>';
			endif;
			
			if( $gg_link != '' ):
				$html .= '<li><a href="'. esc_url( $gg_link ) .'" title="'. esc_attr__( 'Google+', 'floris' ) .'"><i class="fa fa-google-plus"></i></a></li>';
			endif;
			
			if( $pt_link != '' ):
				$html .= '<li><a href="'. esc_url( $pt_link ) .'" title="'. esc_attr__( 'Pinterest', 'floris' ) .'"><i class="fa fa-pinterest"></i></a></li>';
			endif;
		$html .= '</ul></div>';
		endif;
...
**/
function floris_move_comment_field_to_bottom( $fields ) {
...
add_filter( 'comment_form_fields', 'floris_move_comment_field_to_bottom' );