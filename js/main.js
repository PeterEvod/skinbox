(function($) {
	...
	*/
	...
	$('.header-style11 .header-mid .search-icon .fa-search').on('click', function(){
		$('.header-style11 .header-mid .search-cate .floris_top .top-form.top-search').slideToggle();
	});
	...
	*/
	...
	$(document).ready(function(){
		...
		$('#myTabs a').hover(function (e) {
			 e.preventDefault();
			$(this).tab('show');
			 $("li.floris-mega-menu").removeClass("active");
	...
	*/
	...
	if( w_width <= 480){
		jQuery('.mobile-layout .header-mobile-style5 .floris_resmenu')
		.find(' > li:gt(6) ') 
		.hide()
		.end()
		.each(function(){
	...
	*/
	...
	$('#lang_sel ul > li > ul li a').on('click',function(){
	 ...
	 $a = $.cookie('lang_select_floris', $current, { expires: 1, path: '/'}); 
	});
	
	if( $.cookie('lang_select_floris') && $.cookie('lang_select_floris').length > 0 ) {
		$('#lang_sel ul > li > a.lang_sel_sel').html($.cookie('lang_select_floris'));
	}

	$('#lang_sel ul > li.icl-ar').click(function(){
		$('#lang_sel ul > li.icl-en').removeClass( 'active' );
		$(this).addClass( 'active' );
		$.cookie( 'floris_lang_en' , 1, { path: '/', expires: 1 });
	});
	
	$('#lang_sel ul > li.icl-en').click(function(){
		$('#lang_sel ul > li.icl-ar').removeClass( 'active' );
		$(this).addClass( 'active' );
		$.cookie( 'floris_lang_en' , 0, { path: '/', expires: -1 });
	});
	
	var Floris_Lang = $.cookie( 'floris_lang_en' );
	if( Floris_Lang == null ){
	...
	** Clear header style 
	*/
	$( '.floris-logo' ).on('click', function(){
		$.cookie("floris_header_style", null, { path: '/' });
		$.cookie("floris_footer_style", null, { path: '/' });
	});
	...
	**/
	...
	$(window).scroll(function () {
		if ($(this).scrollTop() > whtml/10) {
			...
			$('#floris-totop').fadeOut();
		}
	});
	
	$('#floris-totop').click(function () {
		$('body,html').animate({
			scrollTop: 0
		}, 800);
		return false;
	...
	**/
	if( $('body').html().match( /sw-quickview-bottom/ ) ){
		...
		$(document).on( 'click', 'a[data-type="quickview"]', function(){
			var product_id = $(this).data( 'product_id' ), ajaxurl = $(this).data( 'ajax_url' ).replace( '%%endpoint%%', 'floris_quickviewproduct' );
			ajaxurl = ajaxurl.replace( '%endpoint%', 'floris_quickviewproduct' );
			qv_target.addClass( 'show loading' );
			var data 		= {
				action: 'floris_quickviewproduct',
	...
	*/
	$('form#login_ajax').on('submit', function(e){
		...
		$.ajax({
			...
			data: { 
				'action': 'floris_custom_login_user', //calls wp_ajax_nopriv_ajaxlogin
